...
public final class HttpSendCenter {
	...
	private static final OkHttpClient httpClient = new OkHttpClient.Builder().connectTimeout(40, TimeUnit.SECONDS)
	...
	public static JSONObject postNotBody(String apiKey, String url) {
		Request request = new Request.Builder().url(url).post(RequestBody.create(JSON, new JSONObject().toString()))
	...
	public static JSONObject delete(String apiKey, String url) {
		Request request = new Request.Builder().url(url).delete(RequestBody.create(JSON, new JSONObject().toString()))
				.header("api-key", apiKey).build();
		return handleRequest(request);
	}
