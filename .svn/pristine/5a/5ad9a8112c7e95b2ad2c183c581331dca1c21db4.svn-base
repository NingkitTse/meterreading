...
public class EventReportValue extends UploadData implements Cloneable {
	...
	public Object clone() throws CloneNotSupportedException {
		final EventReportValue clone = (EventReportValue) super.clone();
		EventReportId eventReportId = new EventReportId();
		eventReportId.setSnMeter(this.id.getSnMeter());
		final Date startDateTime = this.id.getStartDateTime();
		if (null != startDateTime) {
			eventReportId.setStartDateTime(new Date(startDateTime.getTime()));
		}
		clone.setId(eventReportId);
		return clone;
	}
